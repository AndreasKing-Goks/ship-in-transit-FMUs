...
class Rudder(Fmi2Slave):
    ...
    def __init__(self, **kwargs):
        ...
        self.max_rudder_angle_degrees_negative      = 0.0
        self.max_rudder_angle_degrees_positive      = 0.0
        ...
        self.register_variable(Real("max_rudder_angle_degrees_negative", causality=Fmi2Causality.parameter,variability=Fmi2Variability.fixed))
        self.register_variable(Real("max_rudder_angle_degrees_positive", causality=Fmi2Causality.parameter,variability=Fmi2Variability.fixed))
    ...
    def get_rudder_force(self, rudder_angle, yaw_angle, forward_speed, vel_current=0.0):
        ...
        '''
        u_current = np.dot(np.linalg.inv(self.rotation(yaw_angle)), vel_current)[0]
        v_force = -self.rudder_angle_to_sway_force_coefficient * rudder_angle * (forward_speed - u_current)
        r_force = -self.rudder_angle_to_yaw_force_coefficient * rudder_angle * (forward_speed - u_current)
        return v_force, r_force
    
    
    def do_step(self, current_time: float, step_size: float) -> bool:
        # Get the input variables
        yaw_angle       = self.yaw_angle_rad
        rudder_angle    = np.deg2rad(self.rudder_angle_deg)
        forward_speed   = self.forward_speed
        current_speed   = self.current_speed
        
        # Saturate the rudder angle
        if rudder_angle < 0:
            rudder_angle = np.max([rudder_angle, self.max_rudder_angle_degrees_negative])
        if rudder_angle >= 0:
            rudder_angle = np.min([rudder_angle, self.max_rudder_angle_degrees_positive])
            
        # Get the Rudder Force
        self.rudder_force_v, self.rudder_force_r = self.get_rudder_force(rudder_angle, yaw_angle, 
