"""
Replay a specific BO trial from CSV results with animation and plots (two target ships).
...
# Fixed scenario decisions (must match main.py)
ENCOUNTERS = [
...
def prepare_scenario(parameters):
...
def replay_trial(trial_index, df, data, save_animation=False):
    ...
    # Replay
    replay_best_trial(
        scenario_preparer_fn=prepare_scenario,
...
def main():
    parser = argparse.ArgumentParser(
        description="Replay BO trials from CSV results (two target ships)",
