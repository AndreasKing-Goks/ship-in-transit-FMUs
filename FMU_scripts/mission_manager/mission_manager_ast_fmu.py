...
class MissionManagerAST(Fmi2Slave):
    ...
    def __init__(self, **kwargs):
        ...
        self.scope_proj_length      = 2500  # 2.5 km
        ...
        self.register_variable(Integer("max_wp", causality=Fmi2Causality.parameter, variability=Fmi2Variability.tunable))
        ...
        self.register_variable(Real("scope_proj_length", causality=Fmi2Causality.parameter, variability=Fmi2Variability.tunable))
        ...
        self.register_variable(Boolean("is_iw_sample", causality=Fmi2Causality.input))
    ...
    def _get_angle_two_points_cartesian(self, base_point, end_point):
        base_north, base_east = base_point
        end_north,  end_east  = end_point
        
        d_north = end_north - base_north
        d_east  = end_east - base_east
        
        return np.atan2(d_north, d_east)
        
    
    def _get_intermediate_waypoint(self, scope_angle_deg):
        # Convert to radian
        scope_angle_rad = np.deg2rad(scope_angle_deg)
        
        # Get the segment angle
        beta = self._get_segment_angle()
        
        # Get the intermediate waypoint
        d_north = 0
        d_east  = 0
        

    def _udpate_trajectory(self):
        return
