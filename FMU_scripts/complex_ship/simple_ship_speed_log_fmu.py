"""
Simple Ship Speed Log Sensor Python FMU implementation.
...
class ShipSpeedLogSensor(Fmi2Slave):
    author = "Andreas R.G. Sitorus"
    description = "Ship Speed Log Sensor Python FMU Implementation"
    ...
    def __init__(self, **kwargs):
        ...
        self.seed                               = 42
        self.ship_speed_sigma                   = 0.05       # [m/s] ~ 0.1 knot
        ...
        self._prev_measured_ship_speed          = 0.0
        
        ## Registration
        # PI Parameters
        self.register_variable(Integer("seed", causality=Fmi2Causality.parameter,variability=Fmi2Variability.fixed))
        self.register_variable(Real("ship_speed_sigma", causality=Fmi2Causality.parameter,variability=Fmi2Variability.tunable))
        
        # Input
        self.register_variable(Boolean("freeze", causality=Fmi2Causality.input, variability=Fmi2Variability.discrete))
        self.register_variable(Real("ship_speed", causality=Fmi2Causality.input))
    ...
    def do_step(self, current_time: float, step_size: float) -> bool:
        try:
            ...
            if not self.freeze:
                ...
                ship_speed_noise                    = self._noise_generator.normal(loc=0.0, scale=self.ship_speed_sigma)
                
                self.measured_ship_speed            = self.ship_speed + ship_speed_noise
            # Sensor stuck
            else:
                self.measured_ship_speed            = self._prev_measured_ship_speed
            
            self._prev_measured_ship_speed          = self.measured_ship_speed
            self.measurement_valid                  = True
        
        except Exception as e:
            # IMPORTANT: do not crash host
            print(f"[ShipSpeedLogSensor] ERROR t={current_time} dt={step_size}: {type(e).__name__}: {e}")
