"""
Simple Gyrocompass Sensor Python FMU implementation.

Authors : Andreas R.G. Sitorus
Date    : September 2026
"""

from pythonfmu import Fmi2Causality, Fmi2Slave, Fmi2Variability, Real, Integer, Boolean, String
import numpy as np
import traceback

class GyrocompassSensor(Fmi2Slave):
    author = "Andreas R.G. Sitorus"
    description = "Gyrocompass Sensor Python FMU Implementation"
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        
        ## Noise Parameters (Sigma = Standard Deviation)
        self.seed                               = 42
        self.ship_heading_sigma_deg             = 0.05
        
        ## Input
        self.freeze                             = False
        self.ship_heading_rad                   = 0.0
        
        ## Output
        self.measured_ship_heading_rad          = 0.0
        self.measurement_valid                  = True
        
        # Internal Variables
        self._noise_generator                   = None
        self._prev_measured_ship_heading_rad    = 0.0
        
        ## Registration
        # PI Parameters
        self.register_variable(Integer("seed", causality=Fmi2Causality.parameter,variability=Fmi2Variability.fixed))
        self.register_variable(Real("ship_heading_sigma_deg", causality=Fmi2Causality.parameter,variability=Fmi2Variability.tunable))
        
        # Input
        self.register_variable(Boolean("freeze", causality=Fmi2Causality.input, variability=Fmi2Variability.discrete))
        self.register_variable(Real("ship_heading_rad", causality=Fmi2Causality.input))

        # Output
        self.register_variable(Real("measured_ship_heading_rad", causality=Fmi2Causality.output))
        self.register_variable(Boolean("measurement_valid", causality=Fmi2Causality.output, variability=Fmi2Variability.discrete))
    
    def _wrap_to_pi(self, a):
        return (a + np.pi) % (2*np.pi) - np.pi
        
    def do_step(self, current_time: float, step_size: float) -> bool:
        try:
            # Precompute once
            if self._noise_generator is None:
                self._noise_generator               = np.random.default_rng(seed=self.seed)
            
            if not self.freeze:
                # Noise only generated when the sensor is not frozen
                ship_heading_rad_noise              = self._noise_generator.normal(loc=0.0, scale=np.deg2rad(self.ship_heading_sigma_deg))
                
                self.measured_ship_heading_rad      = self._wrap_to_pi(self.ship_heading_rad + ship_heading_rad_noise)
            # Sensor stuck
            else:
                self.measured_ship_heading_rad      = self._prev_measured_ship_heading_rad
            
            self._prev_measured_ship_heading_rad    = self.measured_ship_heading_rad
            self.measurement_valid                  = True
        
        except Exception as e:
            # IMPORTANT: do not crash host
            print(f"[GyrocompassSensor] ERROR t={current_time} dt={step_size}: {type(e).__name__}: {e}")
            print(traceback.format_exc())
            
            # Freeze dynamics safely (keep last state/outputs)
            self.measurement_valid                  = False
        
        return True