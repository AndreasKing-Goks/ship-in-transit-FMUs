...
class EBASTv2EnvMultiConfigs(gym.Env):
    """
        ...
        Supports multi configurations settings, does 'config_path' also support lists of
        configurations!
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None, specific_case_idx=None, specific_config_idx=None):
        """
        ...
        for attempt in range(max_attempts):
            ...
            # A list of Paths supports indexed or random selection.
                ...
                # Specific config index can be changed when reset
                elif specific_config_idx is not None:
                    config_idx = specific_config_idx
                # Else do uniform sampling
                else:
                    config_idx = int(self.np_random.integers(0, len(self.config_path)))
